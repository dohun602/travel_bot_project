...
# API 키 설정
openai.api_key = os.getenv("OPENAI_API_KEY") #챗지피티 api
...
def load_airport_ennames():
    ...
    return dict(zip(df["iata_code"], df["name"]))
...
def load_timezone_mapping(filepath="airports.xlsx"):
    try:
...
def calculate_time_difference_by_iata(dep_iata: str, arr_iata: str, timezone_mapping: dict) -> int:
    try:
        ...
        offset_dep = tz_dep.utcoffset(now).total_seconds() / 3600
        offset_arr = tz_arr.utcoffset(now).total_seconds() / 3600
...
def get_hotel_offers(hotel_ids, checkin_date, checkout_date, token):
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = [executor.submit(fetch_offer, hid) for hid in hotel_ids[:10]]
...
def get_lat_lon_from_iata(iata_code):
    df = pd.read_excel("airports.xlsx")  # 업로드한 파일
    row = df[df["IATA Code"] == iata_code]
    ...
    if not recommendations:
        ...
        for i, dest in enumerate(recommendations, 1):
            ...
            if lat and lon:
                ...
                if hotel_info:
                    st.write("🏨 추천 호텔:")
                    for name, price in hotel_info:
                        st.write(f"- {name} - 💰 {price} USD")
                else:
                ...
                if not flight_info:
                    ...
                    for flight in flight_info:
                        ...
                        for seg in segments:
                            ...
                            if dep_time:
                                dep_time_fmt = parser.parse(dep_time).strftime("%Y-%m-%d %H:%M")
                            else:
                                dep_time_fmt = "출발 시각 없음"

                            if arr_time:
                                arr_time_fmt = parser.parse(arr_time).strftime("%Y-%m-%d %H:%M")
                            else:
                                arr_time_fmt = "도착 시각 없음"
